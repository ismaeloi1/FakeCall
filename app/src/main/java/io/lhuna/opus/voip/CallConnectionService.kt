...
class CallConnectionService : ConnectionService() {
    ...
    override fun onCreateIncomingConnection(
        ...
        val number = request.address?.schemeSpecificPart
            ?: extras?.getString(TelecomHelper.EXTRA_CALLER_NUMBER)
            ?: getString(R.string.notification_unknown_caller)
        val name = extras?.getString(TelecomHelper.EXTRA_CALLER_NAME).orEmpty()
        val source = IncomingCallSource.fromStorage(
            extras?.getString(TelecomHelper.EXTRA_CALL_SOURCE)
        )
